...
if(isset($_GET['delete'])){
    $sno = $_GET['delete'];
    $delete = true;
    $sql = "DELETE FROM `jobapplication` WHERE `jobapplication`.`app_id` = $sno";
...
<!--  clearfix -->
            <nav>
                <ul>
                    ...
                    <li  class="current"><a href="profile.php"><i class="icon-line-content-left"></i>Back</a> </li>
...
<table class="table" id="myTable">
      ...
      <tbody>
        <?php 
          $user_id = $_GET['id'];
        // include "../include/config.php";
          $sql = "SELECT * FROM `jobapplication` where u_id = $user_id";
          $result = mysqli_query($conn, $sql);
          $sno = 0;
          while($row = mysqli_fetch_assoc($result)){
            ...
            $jobid = $row['job_id'];
            // echo $jobid;
            $sql2 = "SELECT * FROM `job` where job_id ='$jobid'";
            $result2 = mysqli_query($conn, $sql2);
            $jobs = mysqli_fetch_assoc($result2);
            if (isset($jobs)) {
                echo "<tr>
            <th scope='row'>". $jobs['jobtitle'] . "</th>
            <td>". $jobs['budget'] . "</td>
            <td>". $jobs['contactNo'] . "</td>
            <td>". $jobs['jobdetail'] . "</td>
            <td>". $jobs['address'] . "</td>
            <td>". $row['Status'] . "</td>
            <td> <button class='delete btn btn-sm btn-danger' id=d".$row['app_id'].">Delete</button>  </td>
          </tr>";
            }
            else{
                echo "<tr>
                <th scope='row'>Job id - ". $row['job_id'] . "</th>
...
<script>
    ...
    //       console.log("yes");
    //       window.location = `myapplications.php?edit=${sno}`;
    ...
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
        ...
        if (confirm("Are you sure you want to delete this application!")) {
          console.log("yes");
          window.location = `myapplication.php?delete=${sno}`;
          // TODO: Create a form and use post request to submit a form
        }
        else {
          console.log("no");
